...
export function FlowRunnerWithCoach({ flowJson }: { flowJson: FlowJson }) {
  ...
  useEffect(() => {
    coach.startSession(flowJson.flowName);
  }, [flowJson.flowName, coach]);
  ...
  const processDrillResults = useCallback((step: DrillStep) => {
    ...
    // Calculate DTW tier (simplified)
    if (step.target?.intonation === 'rising') {
      // Simple tier calculation based on end-rise detection and smoothness
      const jitter = drillMetrics.jitterEma || 0;
      const endRise = metrics.endRiseDetected;
      
      if (endRise && jitter < 0.3) {
        metrics.dtwTier = 5;
      } else if (endRise && jitter < 0.5) {
        metrics.dtwTier = 4;
      } else if (endRise || jitter < 0.4) {
        metrics.dtwTier = 3;
      } else {
        metrics.dtwTier = 2;
      }
  ...
  const stopRecording = useCallback(() => {
    ...
    // Process drill results and end step
    if (currentStep?.type === 'drill') {
      processDrillResults(currentStep as DrillStep);
      
      // End step with coach
      const stepResult = coach.endStep(
        currentStep.id,
        currentStep.type,
        true, // success - could be calculated based on metrics
        {
          ...drillMetrics,
          endRiseDetected: drillMetrics.endRiseDetected,
          dtwTier: drillMetrics.dtwTier
        }
      );

      // Announce coach hints
      stepResult.hints.forEach(hint => announceHint(hint));
    }
  }, [currentStep, processDrillResults, coach, drillMetrics, announceHint]);
    ...
    try {
      ...
      processor.onaudioprocess = (event) => {
        ...
        if (result) {
          ...
          // Add metric snapshot to coach
          const snapshot = createMetricSnapshot(
            { ...result, loudNorm: normalizedLoudness },
            timestamp,
            isInTargetRange(result.semitoneRel ?? 0, currentStep as DrillStep),
            false // endRiseDetected will be calculated at the end
          );
  ...
  const isInTargetRange = (semitoneRel: number | null, step: DrillStep | null): boolean => {
    if (!semitoneRel || !step || step.type !== 'drill') return false;
    
    // Define target ranges based on step type
    if (step.target?.intonation === 'rising') {
      // For rising intonation, we want positive semitone values
      return semitoneRel > 0;
    }
    
    // For glide exercises, check if within a reasonable range
    if (step.title?.toLowerCase().includes('glide')) {
      return Math.abs(semitoneRel) < 2; // Within 2 semitones of baseline
    }
  ...
  const nextStep = () => {
    if (!currentStep?.next) {
      ...
      // End coach session
      const coachSummary = coach.endSession();
      console.log('Coach session summary:', coachSummary);
      ...
      case 'drill':
        return (
          <DrillStepComponent 
            ...
            onNext={nextStep}
            coachHints={coach.getCurrentHints()}
          />
        );
      case 'reflection':
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ARIA announcements for coach hints */}
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-8">
      ...
      <div className="mb-8 space-y-2">
        ...
        {metrics.dtwTier !== undefined && (
          <div>Match Quality: {metrics.dtwTier}/5</div>
        )}
      </div>
